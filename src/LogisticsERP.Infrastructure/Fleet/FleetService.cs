...
{
    private const long MaximumReturnEvidenceSize = 10 * 1024 * 1024;
    ...
    public async Task<Result<VehicleDetailResponse>> UpsertVehicleAsync(Guid? id, VehicleUpsertRequest request, CancellationToken cancellationToken = default)
    {
        ...
        var supplierValid = !request.PurchasedFromSupplierId.HasValue
            || await dbContext.VehicleSuppliers.AnyAsync(x => x.Id == request.PurchasedFromSupplierId && x.Status == VehicleCatalogStatus.Active, cancellationToken)
            || id.HasValue && await dbContext.Vehicles.AnyAsync(x => x.Id == id && x.PurchasedFromSupplierId == request.PurchasedFromSupplierId, cancellationToken);
        ...
        if (request.RegisteredOwnerSupplierId.HasValue)
        {
            if (await dbContext.VehicleSuppliers.AnyAsync(x => x.Id == request.RegisteredOwnerSupplierId && x.Status == VehicleCatalogStatus.Active, cancellationToken)
                || id.HasValue && await dbContext.Vehicles.AnyAsync(x => x.Id == id && x.RegisteredOwnerSupplierId == request.RegisteredOwnerSupplierId, cancellationToken))
        ...
        if (request.RegistrationType.HasValue && !Enum.IsDefined(request.RegistrationType.Value)
            || request.CurrentOperationalStatus.HasValue && !Enum.IsDefined(request.CurrentOperationalStatus.Value))
            return Result.Failure<VehicleDetailResponse>(FleetErrors.InvalidRequest);
        ...
        if (vehicle is not null &&
            (!string.Equals(FleetServiceSupport.TrimOrNull(vehicle.SerialNumber), FleetServiceSupport.TrimOrNull(request.SerialNumber), StringComparison.Ordinal)
             || !string.Equals(FleetServiceSupport.TrimOrNull(vehicle.ChassisNumber), FleetServiceSupport.TrimOrNull(request.ChassisNumber), StringComparison.Ordinal)
             || !string.Equals(FleetServiceSupport.TrimOrNull(vehicle.PlateNumberAr), FleetServiceSupport.TrimOrNull(request.PlateNumberAr), StringComparison.Ordinal)
             || !string.Equals(FleetServiceSupport.TrimOrNull(vehicle.PlateNumberEn), FleetServiceSupport.TrimOrNull(request.PlateNumberEn), StringComparison.Ordinal)
             || !string.Equals(FleetServiceSupport.TrimOrNull(vehicle.PlateLettersAr), FleetServiceSupport.TrimOrNull(request.PlateLettersAr), StringComparison.Ordinal)
             || !string.Equals(FleetServiceSupport.TrimOrNull(vehicle.PlateLettersEn), FleetServiceSupport.TrimOrNull(request.PlateLettersEn), StringComparison.Ordinal)
             || !string.Equals(FleetServiceSupport.TrimOrNull(vehicle.PlateDigits), FleetServiceSupport.TrimOrNull(request.PlateDigits), StringComparison.Ordinal)
             || vehicle.RegistrationType != request.RegistrationType)) return Result.Failure<VehicleDetailResponse>(FleetErrors.VehicleIdentityCorrectionRequired);
        ...
        vehicle ??= new Vehicle();
        var requestedStatus = request.CurrentOperationalStatus;
        if (requestedStatus == VehicleOperationalStatus.Decommissioned
            && !await support.HasVehiclePermissionAsync(vehicle, PermissionKeys.Fleet.VehiclesDecommission, cancellationToken))
            return Result.Failure<VehicleDetailResponse>(FleetErrors.Forbidden);
        ApplyVehicle(vehicle, request with { AssetNumber = assetNumber }, registeredOwnerSupplierId, registeredOwnerSponsorId, normalizedAsset, normalizedSerial, normalizedChassis, normalizedAr, normalizedEn, isNew);
        if (isNew)
        {
            var initialStatus = requestedStatus ?? VehicleOperationalStatus.Available;
            if (initialStatus == VehicleOperationalStatus.Assigned)
                return Result.Failure<VehicleDetailResponse>(FleetErrors.InvalidState);
            vehicle.CurrentOperationalStatus = initialStatus;
            if (initialStatus == VehicleOperationalStatus.Decommissioned)
            {
                vehicle.DecommissionedAtUtc = support.UtcNow;
                vehicle.DecommissionReason = "Vehicle created as decommissioned through vehicle upsert.";
            }
            vehicle.TrackedDistanceKm = request.CurrentOdometer;
            dbContext.Vehicles.Add(vehicle);
            dbContext.VehicleOperationalStatusPeriods.Add(NewStatus(vehicle.Id, initialStatus, support.UtcNow, $"Vehicle created with status {initialStatus}.", VehicleStatusSourceType.Vehicle, vehicle.Id, actor.Value));
        ...
        else
        {
            ...
            {
                var statusResult = await ApplyUpsertStatusAsync(
                    vehicle, requestedStatus.Value, actor.Value, cancellationToken);
                if (statusResult.IsFailure)
                    return Result.Failure<VehicleDetailResponse>(statusResult.Error);
            }
            if (dbContext.Entry(vehicle).Property(x => x.CurrentOdometer).OriginalValue != request.CurrentOdometer)
            {
    ...
    private async Task<Result> ApplyUpsertStatusAsync(
        Vehicle vehicle,
        VehicleOperationalStatus target,
        Guid actor,
        CancellationToken cancellationToken)
    {
        var activeAssignment = await dbContext.RiderVehicleAssignments
            .SingleOrDefaultAsync(assignment => assignment.VehicleId == vehicle.Id && assignment.EndedAtUtc == null, cancellationToken);
        if (target == VehicleOperationalStatus.Assigned)
        {
            if (activeAssignment is null) return Result.Failure(FleetErrors.InvalidState);
            vehicle.CurrentAssignmentId = activeAssignment.Id;
        }
        else if (activeAssignment is not null)
        {
            EndAssignment(
                ...
                null,
                $"Vehicle status changed to {target} through vehicle update.",
                actor,
                RiderVehicleAssignmentEventType.Returned);
        }

        var statusChanged = vehicle.CurrentOperationalStatus != target || activeAssignment is not null && target != VehicleOperationalStatus.Assigned;
        if (statusChanged)
        {
            await SetStatusAsync(
                ...
                support.UtcNow,
                $"Vehicle status changed to {target} through vehicle update.",
                VehicleStatusSourceType.Administrative,
                vehicle.Id,
                actor,
                cancellationToken);
        }

        if (target == VehicleOperationalStatus.Decommissioned)
        {
            vehicle.DecommissionedAtUtc ??= support.UtcNow;
            vehicle.DecommissionReason = "Vehicle status changed to Decommissioned through vehicle update.";
        }
        else
        {
            vehicle.DecommissionedAtUtc = null;
            vehicle.DecommissionReason = null;
        }
    ...
    public async Task<Result<VehicleDetailResponse>> CorrectIdentityAsync(Guid id, VehicleIdentityCorrectionRequest request, CancellationToken cancellationToken = default)
    {
        ...
        vehicle.SponsorId = request.SponsorId; vehicle.OperatingCityId = request.OperatingCityId; vehicle.RegistrationType = request.RegistrationType;
        if (request.IsPurchasedFromSupplierIdSpecified) vehicle.PurchasedFromSupplierId = request.PurchasedFromSupplierId;
    ...
    public async Task<Result<VehicleRegistrationTransitionResponse>> TransitionToPublicTransportAsync(Guid id, VehicleRegistrationTransitionRequest request, PrivateFileUpload istimara, PrivateFileUpload operationCard, CancellationToken cancellationToken = default)
    {
        ...
        if (!vehicle.RegistrationType.HasValue) return Result.Failure<VehicleRegistrationTransitionResponse>(FleetErrors.TransitionRegistrationTypeMissing);
        if (string.IsNullOrWhiteSpace(request.PlateNumberAr) || request.PlateNumberAr.Trim().Length > 32) return Result.Failure<VehicleRegistrationTransitionResponse>(FleetErrors.TransitionPlateNumberArInvalid);
        if (string.IsNullOrWhiteSpace(request.PlateNumberEn) || request.PlateNumberEn.Trim().Length > 32) return Result.Failure<VehicleRegistrationTransitionResponse>(FleetErrors.TransitionPlateNumberEnInvalid);
        if (request.PlateLettersAr?.Trim().Length is > 8) return Result.Failure<VehicleRegistrationTransitionResponse>(FleetErrors.TransitionPlateLettersArInvalid);
        if (request.PlateLettersEn?.Trim().Length is > 8) return Result.Failure<VehicleRegistrationTransitionResponse>(FleetErrors.TransitionPlateLettersEnInvalid);
        if (request.PlateDigits?.Trim().Length is > 8) return Result.Failure<VehicleRegistrationTransitionResponse>(FleetErrors.TransitionPlateDigitsInvalid);
        if (string.IsNullOrWhiteSpace(request.Reason) || request.Reason.Trim().Length > 1000) return Result.Failure<VehicleRegistrationTransitionResponse>(FleetErrors.TransitionReasonInvalid);
        if (request.EffectiveAtUtc == default) return Result.Failure<VehicleRegistrationTransitionResponse>(FleetErrors.TransitionEffectiveDateRequired);
        if (istimara.Length is <= 0 or > 10 * 1024 * 1024 || !IsDocument(istimara)) return Result.Failure<VehicleRegistrationTransitionResponse>(FleetErrors.TransitionIstimaraInvalid);
        if (operationCard.Length is <= 0 or > 10 * 1024 * 1024 || !IsDocument(operationCard)) return Result.Failure<VehicleRegistrationTransitionResponse>(FleetErrors.TransitionOperationCardInvalid);
        var normalizedAr = FleetServiceSupport.NormalizeIdentifier(request.PlateNumberAr);
        var normalizedEn = FleetServiceSupport.NormalizeIdentifier(request.PlateNumberEn);
        if (await dbContext.Vehicles.AnyAsync(x => x.Id != id && x.NormalizedPlateNumberAr == normalizedAr, cancellationToken)) return Result.Failure<VehicleRegistrationTransitionResponse>(FleetErrors.TransitionDuplicatePlateNumberAr);
        if (await dbContext.Vehicles.AnyAsync(x => x.Id != id && x.NormalizedPlateNumberEn == normalizedEn, cancellationToken)) return Result.Failure<VehicleRegistrationTransitionResponse>(FleetErrors.TransitionDuplicatePlateNumberEn);
        var actor = support.UserId;
        ...
        if (istimaraStaged.IsFailure) return Result.Failure<VehicleRegistrationTransitionResponse>(FleetErrors.TransitionIstimaraInvalid);
        ...
        if (operationCardStaged.IsFailure) { fileStorage.DeleteBestEffort(istimaraStaged.Value!.Stored.StoragePath); return Result.Failure<VehicleRegistrationTransitionResponse>(FleetErrors.TransitionOperationCardInvalid); }
        ...
        try
        {
            return await dbContext.ExecuteTransactionAsync(async _ =>
            {
                ...
                var transition = new VehicleRegistrationTransition
                {
                    ...
                    OldPlateNumberAr = vehicle.PlateNumberAr ?? string.Empty, OldPlateNumberEn = vehicle.PlateNumberEn ?? string.Empty, NewPlateNumberAr = request.PlateNumberAr.Trim(), NewPlateNumberEn = request.PlateNumberEn.Trim(),
    ...
    {
        var reason = DefaultReason(request.Reason);
        ...
        var hash = FleetServiceSupport.HashRequest(new { Request = request with { Reason = reason }, Files = staged.Select(x => x.Stored.Sha256Checksum).ToArray() });
        ...
        try
        {
            return await dbContext.ExecuteTransactionAsync(async _ =>
            {
                ...
                var assignment = new RiderVehicleAssignment
                {
                    ...
                    PermissionStartsOn = permitStart, PermissionEndsOn = FleetBusinessRules.PermitEnd(permitStart), AssignmentReason = reason, AssignedByUserId = actor.Value,
                    WasBackdated = request.StartedAtUtc < support.UtcNow.AddMinutes(-5), BackdatedReason = request.StartedAtUtc < support.UtcNow.AddMinutes(-5) ? reason : null, Notes = FleetServiceSupport.TrimOrNull(request.Notes)
                };
                ...
                dbContext.RiderVehicleAssignmentEvents.Add(NewAssignmentEvent(assignment.Id, operationId, eventType, request.StartedAtUtc, actor.Value, reason));
                if (request.StartOdometer > vehicle.CurrentOdometer)
                {
                    ...
                    dbContext.VehicleOdometerReadings.Add(NewOdometer(vehicle.Id, request.StartOdometer, request.StartedAtUtc, VehicleOdometerSourceType.AssignmentTake, assignment.Id, reason));
                }
    ...
    public async Task<Result<RiderVehicleAssignmentResponse>> ReturnAsync(ReturnVehicleRequest request, IReadOnlyList<PrivateFileUpload> evidenceFiles, string idempotencyKey, CancellationToken cancellationToken = default)
    {
        var reason = DefaultReason(request.Reason);
        if (string.IsNullOrWhiteSpace(idempotencyKey)) return Result.Failure<RiderVehicleAssignmentResponse>(FleetErrors.IdempotencyRequired);
        if (!Enum.IsDefined(request.EndCondition)) return Result.Failure<RiderVehicleAssignmentResponse>(FleetErrors.InvalidRequest);
        var needsReport = FleetBusinessRules.RequiresReturnConditionReport(request.EndCondition);
        if (!FleetBusinessRules.IsValidReturnConditionReport(
                request.EndCondition,
                request.ConditionReport is not null,
                request.ConditionReport?.Category,
                request.ConditionReport?.Severity,
                request.ConditionReport?.ProblemDescription,
                request.ConditionReport?.EstimatedRepairCost,
                evidenceFiles.Count))
        {
            return Result.Failure<RiderVehicleAssignmentResponse>(needsReport
                ? FleetErrors.ReturnConditionReportRequired
                : FleetErrors.ReturnConditionReportNotAllowed);
        }

        var issueId = Guid.CreateVersion7();
        var stagedResult = await StageIssueEvidenceAsync(issueId, evidenceFiles, cancellationToken);
        if (stagedResult.IsFailure) return Result.Failure<RiderVehicleAssignmentResponse>(stagedResult.Error);
        var staged = stagedResult.Value!;
        var normalizedRequest = request with { Reason = reason };
        var hash = evidenceFiles.Count == 0
            ? FleetServiceSupport.HashRequest(normalizedRequest)
            : FleetServiceSupport.HashRequest(new { Request = normalizedRequest, Files = staged.Select(x => x.Stored.Sha256Checksum).ToArray() });
        var replay = await ReplayAssignmentAsync("return", idempotencyKey, hash, cancellationToken);
        if (replay is not null) { CleanupIssueEvidence(staged); return replay; }
        var actor = support.UserId;
        if (!actor.HasValue) { CleanupIssueEvidence(staged); return Result.Failure<RiderVehicleAssignmentResponse>(FleetErrors.CurrentUserUnavailable); }
        ...
        try
        {
            return await dbContext.ExecuteTransactionAsync(async _ =>
            {
                EndAssignment(assignment, vehicle, request.EndedAtUtc, request.EndOdometer, request.EndCondition, request.EndFuelLevelPercentage, reason, actor.Value, RiderVehicleAssignmentEventType.Returned);
                if (needsReport)
                {
                    var report = request.ConditionReport!;
                    var issue = new VehicleIssue
                    {
                        Id = issueId,
                        IssueNumber = FleetServiceSupport.NewIssueNumber(issueId),
                        VehicleId = vehicle.Id,
                        Category = report.Category,
                        Severity = report.Severity,
                        Description = report.ProblemDescription.Trim(),
                        ReportedAtUtc = request.EndedAtUtc,
                        OdometerAtReport = request.EndOdometer,
                        RelatedAssignmentId = assignment.Id,
                        IsRiderResponsible = report.IsRiderResponsible,
                    ...
                    dbContext.VehicleIssueEvents.Add(NewIssueEvent(issue.Id, VehicleIssueEventType.Reported, null, VehicleIssueStatus.Open, request.EndedAtUtc, actor.Value, report.ProblemDescription));
                    AddStagedIssueEvidence(issue.Id, staged, actor.Value);
                    await SetStatusAsync(vehicle, VehicleOperationalStatus.ProblemHold, request.EndedAtUtc, report.ProblemDescription, VehicleStatusSourceType.Issue, issue.Id, actor.Value, cancellationToken);
                }
                else
                {
    ...
    public async Task<Result<RiderVehicleAssignmentResponse>> SwitchAsync(SwitchVehicleRequest request, IReadOnlyList<PrivateFileUpload> promissoryFiles, IReadOnlyList<PrivateFileUpload> evidenceFiles, string idempotencyKey, CancellationToken cancellationToken = default)
    {
        if (string.IsNullOrWhiteSpace(idempotencyKey)) return Result.Failure<RiderVehicleAssignmentResponse>(FleetErrors.IdempotencyRequired);
        if (promissoryFiles.Count > 3) return Result.Failure<RiderVehicleAssignmentResponse>(FleetErrors.FileLimit);
        if (!Enum.IsDefined(request.OldVehicleCondition) || !Enum.IsDefined(request.NewVehicleCondition)) return Result.Failure<RiderVehicleAssignmentResponse>(FleetErrors.InvalidRequest);
        var needsReport = FleetBusinessRules.RequiresReturnConditionReport(request.OldVehicleCondition);
        if (!FleetBusinessRules.IsValidReturnConditionReport(
                request.OldVehicleCondition,
                request.ConditionReport is not null,
                request.ConditionReport?.Category,
                request.ConditionReport?.Severity,
                request.ConditionReport?.ProblemDescription,
        ...
        var promissoryResult = await StagePromissoryFilesAsync(oldForRider.RiderProfileId, promissoryFiles, cancellationToken);
        if (promissoryResult.IsFailure) return Result.Failure<RiderVehicleAssignmentResponse>(promissoryResult.Error);
        var stagedPromissory = promissoryResult.Value!;
        var issueId = Guid.CreateVersion7();
        var evidenceResult = await StageIssueEvidenceAsync(issueId, evidenceFiles, cancellationToken);
        if (evidenceResult.IsFailure)
        {
            CleanupStaged(stagedPromissory);
            return Result.Failure<RiderVehicleAssignmentResponse>(evidenceResult.Error);
        }
        var stagedEvidence = evidenceResult.Value!;
        var hash = FleetServiceSupport.HashRequest(new
        {
            Request = request,
            PromissoryFiles = stagedPromissory.Select(x => x.Stored.Sha256Checksum).ToArray(),
            EvidenceFiles = stagedEvidence.Select(x => x.Stored.Sha256Checksum).ToArray()
        });
        var replay = await ReplayAssignmentAsync("switch", idempotencyKey, hash, cancellationToken);
        if (replay is not null) { CleanupSwitchFiles(stagedPromissory, stagedEvidence); return replay; }
        var actor = support.UserId;
        if (!actor.HasValue) { CleanupSwitchFiles(stagedPromissory, stagedEvidence); return Result.Failure<RiderVehicleAssignmentResponse>(FleetErrors.CurrentUserUnavailable); }
        ...
        if (existingPromissoryVersions.Count + stagedPromissory.Count > 3) { CleanupSwitchFiles(stagedPromissory, stagedEvidence); return Result.Failure<RiderVehicleAssignmentResponse>(FleetErrors.FileLimit); }
        try
        {
            return await dbContext.ExecuteTransactionAsync(async _ =>
            {
                ...
                if (needsReport)
                {
                    ...
                    var issue = new VehicleIssue
                    {
                        ...
                        Severity = report.Severity,
                        Description = report.ProblemDescription.Trim(),
                        ReportedAtUtc = request.SwitchedAtUtc,
                        OdometerAtReport = request.OldVehicleOdometer,
                        RelatedAssignmentId = old.Id,
                        IsRiderResponsible = report.IsRiderResponsible,
                        EstimatedRepairCost = report.EstimatedRepairCost,
                        BlocksOperation = true,
                        ReportedByUserId = actor.Value
                    ...
                    dbContext.VehicleIssues.Add(issue);
                    dbContext.VehicleIssueEvents.Add(NewIssueEvent(issue.Id, VehicleIssueEventType.Reported, null, VehicleIssueStatus.Open, request.SwitchedAtUtc, actor.Value, report.ProblemDescription));
                    AddStagedIssueEvidence(issue.Id, stagedEvidence, actor.Value);
                    await SetStatusAsync(oldVehicle, VehicleOperationalStatus.ProblemHold, request.SwitchedAtUtc, report.ProblemDescription, VehicleStatusSourceType.Issue, issue.Id, actor.Value, cancellationToken);
                }
                else
                {
                ...
                var versions = existingPromissoryVersions
                    .Concat(AddStagedPromissoryFiles(old.RiderProfileId, stagedPromissory, actor.Value))
                    .ToArray();
                ...
                if (stagedPromissory.Count > 0) await dbContext.SaveChangesAsync(cancellationToken);
                return Result.Success(await MapAssignmentAsync(newAssignment, cancellationToken));
            }, cancellationToken);
        }
        catch (DbUpdateException)
        {
            CleanupSwitchFiles(stagedPromissory, stagedEvidence);
            return Result.Failure<RiderVehicleAssignmentResponse>(FleetErrors.Conflict);
        }
        catch
        {
    ...
    public async Task<Result<RiderVehicleAssignmentResponse>> GetAssignmentAsync(Guid assignmentId, CancellationToken cancellationToken = default)
    {
        var assignment = await dbContext.RiderVehicleAssignments.AsNoTracking().SingleOrDefaultAsync(x => x.Id == assignmentId, cancellationToken);
        if (assignment is null) return Result.Failure<RiderVehicleAssignmentResponse>(FleetErrors.NotFound);
        ...
        if (!await support.HasVehiclePermissionAsync(vehicle, PermissionKeys.Fleet.AssignmentsRead, cancellationToken))
            return Result.Failure<RiderVehicleAssignmentResponse>(FleetErrors.Forbidden);
        return Result.Success(await MapAssignmentAsync(assignment, cancellationToken));
    }

    public async Task<Result<RiderVehicleAssignmentResponse>> AttachPromissoryFilesAsync(Guid assignmentId, string rowVersion, IReadOnlyList<PrivateFileUpload> promissoryFiles, string idempotencyKey, CancellationToken cancellationToken = default)
    {
        if (string.IsNullOrWhiteSpace(idempotencyKey)) return Result.Failure<RiderVehicleAssignmentResponse>(FleetErrors.IdempotencyRequired);
        if (promissoryFiles.Count == 0) return Result.Failure<RiderVehicleAssignmentResponse>(FleetErrors.PromissoryFilesRequired);
        if (promissoryFiles.Count > 3) return Result.Failure<RiderVehicleAssignmentResponse>(FleetErrors.FileLimit);
        if (string.IsNullOrWhiteSpace(rowVersion)) return Result.Failure<RiderVehicleAssignmentResponse>(FleetErrors.ConcurrencyConflict);

        var assignment = await dbContext.RiderVehicleAssignments.SingleOrDefaultAsync(x => x.Id == assignmentId, cancellationToken);
        if (assignment is null) return Result.Failure<RiderVehicleAssignmentResponse>(FleetErrors.NotFound);
        var vehicle = await dbContext.Vehicles.AsNoTracking().SingleOrDefaultAsync(x => x.Id == assignment.VehicleId, cancellationToken);
        if (vehicle is null) return Result.Failure<RiderVehicleAssignmentResponse>(FleetErrors.NotFound);
        ...
        var actor = support.UserId;
        if (!actor.HasValue) return Result.Failure<RiderVehicleAssignmentResponse>(FleetErrors.CurrentUserUnavailable);
        var stagedResult = await StagePromissoryFilesAsync(assignment.RiderProfileId, promissoryFiles, cancellationToken);
        if (stagedResult.IsFailure) return Result.Failure<RiderVehicleAssignmentResponse>(stagedResult.Error);
        var staged = stagedResult.Value!;
        var hash = FleetServiceSupport.HashRequest(new { AssignmentId = assignmentId, RowVersion = rowVersion, Files = staged.Select(x => x.Stored.Sha256Checksum).ToArray() });
        var replay = await ReplayAssignmentAsync("attach-promissory-files", idempotencyKey.Trim(), hash, cancellationToken);
        if (replay is not null) { CleanupStaged(staged); return replay; }
        if (assignment.EndedAtUtc.HasValue || assignment.Status != RiderVehicleAssignmentStatus.Active || vehicle.CurrentAssignmentId != assignment.Id)
        { CleanupStaged(staged); return Result.Failure<RiderVehicleAssignmentResponse>(FleetErrors.AssignmentNotActive); }
        if (!FleetServiceSupport.MatchesRowVersion(assignment.RowVersion, rowVersion))
        { CleanupStaged(staged); return Result.Failure<RiderVehicleAssignmentResponse>(FleetErrors.ConcurrencyConflict); }
        var existingVersions = await CurrentPromissoryVersionsAsync(assignment.RiderProfileId, cancellationToken);
        if (existingVersions.Count + staged.Count > 3)
        { CleanupStaged(staged); return Result.Failure<RiderVehicleAssignmentResponse>(FleetErrors.FileLimit); }
        try
        {
            return await dbContext.ExecuteTransactionAsync(async _ =>
            {
                var versionIds = AddStagedPromissoryFiles(assignment.RiderProfileId, staged, actor.Value);
                foreach (var versionId in versionIds)
                    dbContext.RiderVehicleAssignmentPromissoryFiles.Add(new RiderVehicleAssignmentPromissoryFile { RiderVehicleAssignmentId = assignment.Id, RiderPromissoryFileVersionId = versionId });
                assignment.UpdatedAtUtc = support.UtcNow;
                assignment.UpdatedByUserId = actor.Value;
                dbContext.Entry(assignment).Property(x => x.UpdatedAtUtc).IsModified = true;
                dbContext.FleetCommandReceipts.Add(new FleetCommandReceipt { CommandName = "attach-promissory-files", IdempotencyKey = idempotencyKey.Trim(), RequestHash = hash, ResultEntityId = assignment.Id });
                await dbContext.SaveChangesAsync(cancellationToken);
                ActivateStagedPromissoryFiles(staged);
                await dbContext.SaveChangesAsync(cancellationToken);
                return Result.Success(await MapAssignmentAsync(assignment, cancellationToken));
        ...
        {
            CleanupStaged(staged);
            return Result.Failure<RiderVehicleAssignmentResponse>(FleetErrors.ConcurrencyConflict);
        }
        catch (DbUpdateException)
        {
            CleanupStaged(staged);
    ...
    private async Task<Result<IReadOnlyList<RiderVehicleTimelineResponse>>> GetTimelineAsync(Guid? vehicleId, Guid? riderId, CancellationToken cancellationToken)
    {
        ...
        foreach (var assignment in assignments)
        {
            ...
            var assignmentResponse = await MapAssignmentAsync(assignment, cancellationToken);
            var rider = new VehicleIssueRiderResponse(assignmentResponse.RiderProfileId, assignmentResponse.EmployeeId, assignmentResponse.RiderName, assignmentResponse.IsRealRider, assignmentResponse.RealRider);
            result.Add(new RiderVehicleTimelineResponse(assignmentResponse, issues.Select(issue => MapIssue(issue, rider)).ToArray(), accidents.Select(MapAccident).ToArray()));
        }
    ...
    public async Task<Result<PagedResponse<VehicleIssueSummaryResponse>>> GetIssuesAsync(Guid? vehicleId, string? status, int page, int pageSize, CancellationToken cancellationToken = default)
    {
        ...
        var responses = await BuildIssueResponsesAsync(items, cancellationToken);
        return Result.Success(new PagedResponse<VehicleIssueSummaryResponse>(responses, page, pageSize, count));
    }

    public async Task<Result<IReadOnlyList<VehicleIssueEvidenceResponse>>> GetIssueEvidenceAsync(Guid issueId, CancellationToken cancellationToken = default)
    {
        var issue = await dbContext.VehicleIssues.AsNoTracking().SingleOrDefaultAsync(x => x.Id == issueId, cancellationToken);
        if (issue is null) return Result.Failure<IReadOnlyList<VehicleIssueEvidenceResponse>>(FleetErrors.NotFound);
        var access = await GetAccessibleVehicleAsync(issue.VehicleId, PermissionKeys.Fleet.IssuesRead, cancellationToken);
        if (access.IsFailure) return Result.Failure<IReadOnlyList<VehicleIssueEvidenceResponse>>(access.Error);
        var files = await dbContext.VehicleIssueEvidenceFiles.AsNoTracking()
            .Where(x => x.VehicleIssueId == issueId)
            .OrderBy(x => x.UploadedAtUtc)
            .ToArrayAsync(cancellationToken);
        return Result.Success<IReadOnlyList<VehicleIssueEvidenceResponse>>(files.Select(MapIssueEvidence).ToArray());
    }

    public async Task<Result<PrivateFileDownload>> DownloadIssueEvidenceAsync(Guid issueId, Guid evidenceId, CancellationToken cancellationToken = default)
    {
        var evidence = await dbContext.VehicleIssueEvidenceFiles.IgnoreQueryFilters().AsNoTracking()
            .SingleOrDefaultAsync(x => x.Id == evidenceId && x.VehicleIssueId == issueId, cancellationToken);
        if (evidence is null) return Result.Failure<PrivateFileDownload>(FleetErrors.NotFound);
        var issue = await dbContext.VehicleIssues.IgnoreQueryFilters().AsNoTracking().SingleOrDefaultAsync(x => x.Id == issueId, cancellationToken);
        if (issue is null) return Result.Failure<PrivateFileDownload>(FleetErrors.NotFound);
        var access = await GetAccessibleVehicleAsync(issue.VehicleId, PermissionKeys.Fleet.IssuesRead, cancellationToken);
        if (access.IsFailure) return Result.Failure<PrivateFileDownload>(access.Error);
        var file = await fileStorage.OpenReadAsync(evidence.StoragePath, evidence.ContentType, evidence.OriginalFileName, evidence.FileSizeBytes, cancellationToken);
    ...
    public async Task<Result<VehicleIssueSummaryResponse>> CreateIssueAsync(CreateVehicleIssueRequest request, string idempotencyKey, CancellationToken cancellationToken = default)
    {
        ...
        if (receipt is not null)
        {
            ...
            return Result.Success(await BuildIssueResponseAsync(existing, cancellationToken));
        }
        var vehicle = await dbContext.Vehicles.SingleOrDefaultAsync(x => x.Id == request.VehicleId, cancellationToken);
        ...
        var issue = new VehicleIssue { IssueNumber = FleetServiceSupport.NewIssueNumber(Guid.CreateVersion7()), VehicleId = vehicle.Id, Category = request.Category, Severity = request.Severity, Description = request.Description.Trim(), ReportedAtUtc = request.ReportedAtUtc, LocationDescription = FleetServiceSupport.TrimOrNull(request.LocationDescription), OdometerAtReport = request.OdometerAtReport, RelatedAssignmentId = assignment?.Id, BlocksOperation = request.BlocksOperation, ReportedByUserId = actor.Value };
        dbContext.VehicleIssues.Add(issue);
        ...
        await dbContext.SaveChangesAsync(cancellationToken);
        return Result.Success(await BuildIssueResponseAsync(issue, cancellationToken));
    }
    ...
    {
        var info = await (from v in dbContext.Vehicles.AsNoTracking()
                          join r in dbContext.RiderProfiles.AsNoTracking() on item.RiderProfileId equals r.Id
                          join e in dbContext.Employees.AsNoTracking() on r.EmployeeId equals e.Id
                          where v.Id == item.VehicleId
                          select new
                          {
                              v.AssetNumber,
                              ...
                              VehicleOperatingCityNameAr = (
                                  from operatingCity in dbContext.OperatingCities.AsNoTracking()
                                  join city in dbContext.GlobalCities.AsNoTracking() on operatingCity.GlobalCityId equals city.Id
                                  where operatingCity.Id == v.OperatingCityId
    ...
    private async Task<VehicleIssueSummaryResponse> BuildIssueResponseAsync(VehicleIssue issue, CancellationToken cancellationToken) =>
        (await BuildIssueResponsesAsync([issue], cancellationToken))[0];

    private async Task<VehicleIssueSummaryResponse[]> BuildIssueResponsesAsync(VehicleIssue[] issues, CancellationToken cancellationToken)
    {
        if (issues.Length == 0) return [];

        var assignmentIds = issues
            .Where(x => x.RelatedAssignmentId.HasValue)
            .Select(x => x.RelatedAssignmentId!.Value)
            .Distinct()
            .ToArray();

        var riderRows = await (
            from assignment in dbContext.RiderVehicleAssignments.AsNoTracking()
            join profile in dbContext.RiderProfiles.AsNoTracking() on assignment.RiderProfileId equals profile.Id
            join employee in dbContext.Employees.AsNoTracking() on profile.EmployeeId equals employee.Id
            join realRiderRow in dbContext.RealRiders.AsNoTracking() on assignment.Id equals realRiderRow.RiderVehicleAssignmentId into realRiders
        ...
        var ridersByAssignmentId = riderRows.ToDictionary(
            x => x.AssignmentId,
            x => new VehicleIssueRiderResponse(
                x.RiderProfileId,
                x.EmployeeId,
                x.RiderName,
                x.IsRealRider,
                x.IsRealRider || !x.RealRiderId.HasValue
                    ? null
                    : new RealRiderResponse(x.RealRiderId.Value, x.RealRiderName!, x.RealRiderIqamaNo!, x.RealRiderRelationship!)));

        return issues.Select(issue =>
        {
            VehicleIssueRiderResponse? rider = null;
            if (issue.RelatedAssignmentId.HasValue)
            {
                ridersByAssignmentId.TryGetValue(issue.RelatedAssignmentId.Value, out rider);
            }
            return MapIssue(issue, rider);
        }).ToArray();
    }
    ...
    {
        v.AssetNumber = r.AssetNumber!.Trim(); v.NormalizedAssetNumber = normalizedAsset;
        if (isNew)
        {
            v.SerialNumber = FleetServiceSupport.TrimOrNull(r.SerialNumber); v.NormalizedSerialNumber = normalizedSerial;
            v.PlateNumberAr = FleetServiceSupport.TrimOrNull(r.PlateNumberAr); v.NormalizedPlateNumberAr = normalizedAr;
            v.PlateNumberEn = FleetServiceSupport.TrimOrNull(r.PlateNumberEn); v.NormalizedPlateNumberEn = normalizedEn;
            v.PlateLettersAr = FleetServiceSupport.TrimOrNull(r.PlateLettersAr); v.PlateLettersEn = FleetServiceSupport.TrimOrNull(r.PlateLettersEn); v.PlateDigits = FleetServiceSupport.TrimOrNull(r.PlateDigits);
            v.ChassisNumber = FleetServiceSupport.TrimOrNull(r.ChassisNumber); v.NormalizedChassisNumber = normalizedChassis;
            v.RegistrationType = r.RegistrationType;
        }
        v.Vin = FleetServiceSupport.TrimOrNull(r.Vin)?.ToUpperInvariant(); v.EngineNumber = FleetServiceSupport.TrimOrNull(r.EngineNumber);
        v.SponsorId = r.SponsorId; v.OperatingCityId = r.OperatingCityId; v.RegisteredOwnerSupplierId = registeredOwnerSupplierId; v.RegisteredOwnerSponsorId = registeredOwnerSponsorId;
        if (r.IsPurchasedFromSupplierIdSpecified) v.PurchasedFromSupplierId = r.PurchasedFromSupplierId;
    ...
    private static string DefaultReason(string? reason) => FleetServiceSupport.TrimOrNull(reason) ?? "Not provided.";
    ...
    private static VehicleIssueSummaryResponse MapIssue(VehicleIssue x, VehicleIssueRiderResponse? rider) => new(x.Id, x.IssueNumber, x.VehicleId, x.Category, x.Severity, x.BlocksOperation, x.Status, x.ReportedAtUtc, x.Description, x.LocationDescription, x.RelatedAssignmentId, rider, x.IsRiderResponsible, x.EstimatedRepairCost, x.ResolutionSummary, FleetServiceSupport.EncodeRowVersion(x.RowVersion));
    private static VehicleIssueEvidenceResponse MapIssueEvidence(VehicleIssueEvidence x) => new(x.Id, x.VehicleIssueId, x.OriginalFileName, x.ContentType, x.FileSizeBytes, x.Sha256Checksum, x.UploadedAtUtc, FleetServiceSupport.EncodeRowVersion(x.RowVersion));
    ...
    private async Task<Result<List<StagedIssueEvidence>>> StageIssueEvidenceAsync(Guid issueId, IReadOnlyList<PrivateFileUpload> uploads, CancellationToken cancellationToken)
    {
        var result = new List<StagedIssueEvidence>(uploads.Count);
        foreach (var upload in uploads)
        {
            if (!IsDocument(upload)) { CleanupIssueEvidence(result); return Result.Failure<List<StagedIssueEvidence>>(FleetErrors.InvalidFile); }
            var evidenceId = Guid.CreateVersion7();
            var stored = await fileStorage.StoreAsync($"vehicle-issues/{issueId:N}/evidence/{evidenceId:N}", upload, MaximumReturnEvidenceSize, cancellationToken);
            if (stored.IsFailure) { CleanupIssueEvidence(result); return Result.Failure<List<StagedIssueEvidence>>(FleetErrors.InvalidFile); }
            result.Add(new StagedIssueEvidence(evidenceId, stored.Value!));
        }
        return Result.Success(result);
    }

    private void AddStagedIssueEvidence(Guid issueId, IReadOnlyList<StagedIssueEvidence> staged, Guid actor)
    {
        foreach (var item in staged)
        {
            dbContext.VehicleIssueEvidenceFiles.Add(new VehicleIssueEvidence
            {
                Id = item.EvidenceId,
                VehicleIssueId = issueId,
                OriginalFileName = item.Stored.OriginalFileName,
                StoredFileName = item.Stored.StoredFileName,
                ContentType = item.Stored.ContentType,
                FileSizeBytes = item.Stored.Length,
                Sha256Checksum = item.Stored.Sha256Checksum,
                StoragePath = item.Stored.StoragePath,
                UploadedByUserId = actor,
                UploadedAtUtc = support.UtcNow
            });
        }
    }
    ...
    private void CleanupIssueEvidence(IEnumerable<StagedIssueEvidence> staged)
    {
        foreach (var item in staged) fileStorage.DeleteBestEffort(item.Stored.StoragePath);
    }

    private void CleanupSwitchFiles(IEnumerable<StagedPromissoryFile> promissoryFiles, IEnumerable<StagedIssueEvidence> evidenceFiles)
    {
        CleanupStaged(promissoryFiles);
        CleanupIssueEvidence(evidenceFiles);
    }
    ...
    private sealed record StagedIssueEvidence(Guid EvidenceId, StoredPrivateFile Stored);
