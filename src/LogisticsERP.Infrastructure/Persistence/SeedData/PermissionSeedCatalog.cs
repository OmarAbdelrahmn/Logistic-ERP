...
internal static class PermissionSeedCatalog
{
    ...
    [
        ...
        Create(56, PermissionKeys.Fleet.VehiclesRead, "Fleet", "عرض المركبات", "Read vehicles", "عرض المركبات ومواقعها وحالتها.", "View vehicles, locations, and operational status.", housingScope: true),
        Create(57, PermissionKeys.Fleet.VehiclesManage, "Fleet", "إدارة المركبات", "Manage vehicles", "إنشاء وتعديل المركبات وحالتها التشغيلية.", "Create and update vehicles and their operational status.", housingScope: true),
        ...
        Create(60, PermissionKeys.Fleet.AssignmentsRead, "Fleet", "عرض عهد المركبات", "Read vehicle assignments", "عرض العهد الحالية والتاريخية بين الرايدرز والمركبات.", "View current and historical rider-vehicle assignments.", housingScope: true),
        Create(61, PermissionKeys.Fleet.AssignmentsManage, "Fleet", "إدارة عهد المركبات", "Manage vehicle assignments", "تنفيذ الاستلام والإرجاع والتبديل وتجديد التصريح.", "Execute take, return, switch, and permission renewal.", housingScope: true),
        ...
        Create(63, PermissionKeys.Fleet.IssuesRead, "Fleet", "عرض بلاغات المركبات", "Read vehicle issues", "عرض بلاغات وأعطال المركبات.", "View vehicle issues and faults.", housingScope: true),
        Create(64, PermissionKeys.Fleet.IssuesManage, "Fleet", "إدارة بلاغات المركبات", "Manage vehicle issues", "تسجيل ومراجعة وحل وإغلاق البلاغات.", "Report, review, resolve, and close vehicle issues.", housingScope: true),
        Create(65, PermissionKeys.Fleet.ComplianceRead, "Fleet", "عرض التزام المركبات", "Read vehicle compliance", "عرض التسجيل والتأمين والفحص الدوري.", "View vehicle registration, insurance, and inspection.", housingScope: true),
        Create(66, PermissionKeys.Fleet.ComplianceManage, "Fleet", "إدارة التزام المركبات", "Manage vehicle compliance", "إضافة وتجديد وثائق التزام المركبات.", "Add and renew vehicle compliance records.", housingScope: true),
        Create(67, PermissionKeys.Fleet.FilesRead, "Fleet", "عرض ملفات المركبات", "Read vehicle files", "عرض بيانات ونسخ ملفات المركبات.", "View vehicle file metadata and versions.", housingScope: true, sensitive: true),
        Create(68, PermissionKeys.Fleet.FilesUpload, "Fleet", "رفع ملفات المركبات", "Upload vehicle files", "رفع وأرشفة نسخ ملفات المركبات.", "Upload and archive vehicle file versions.", housingScope: true, sensitive: true),
        Create(69, PermissionKeys.Fleet.FilesDownload, "Fleet", "تنزيل ملفات المركبات", "Download vehicle files", "تنزيل محتوى ملفات المركبات الخاصة.", "Download private vehicle file content.", housingScope: true, sensitive: true),
        Create(70, PermissionKeys.Fleet.AccidentsRead, "Fleet", "عرض حوادث المركبات", "Read vehicle accidents", "عرض بيانات الحوادث والأدلة.", "View vehicle accidents and evidence.", housingScope: true, sensitive: true),
        Create(71, PermissionKeys.Fleet.AccidentsReport, "Fleet", "تسجيل حوادث المركبات", "Report vehicle accidents", "تسجيل حادث مرتبط برايدر وعهدة فعالة.", "Report an accident linked to a rider and active assignment.", housingScope: true, sensitive: true),
        ...
        Create(73, PermissionKeys.Fleet.AccidentsDownload, "Fleet", "تنزيل تقارير الحوادث", "Download accident reports", "تنزيل الأدلة وتقارير الحوادث الخاصة.", "Download private accident evidence and reports.", housingScope: true, sensitive: true),
        Create(74, PermissionKeys.Fleet.CorrectionsManage, "Fleet", "تصحيح بيانات الأسطول", "Manage fleet corrections", "تنفيذ تصحيحات العداد والحالة عالية الثقة.", "Perform high-trust odometer and status corrections.", sensitive: true, highTrust: true),
