...
SHEETS = WORKSPACE / "tmp" / "amazon_contact_sheet" / "a4_sheets"
OUTPUT = WORKSPACE / "output" / "Amazon_front_pages_6_per_page.docx"

# A4 landscape (11.69 x 8.27 in), with six upright cards per page.
CANVAS_SIZE = (3508, 2200)
CARD_WIDTH = 1070
CARD_HEIGHT = 677
LEFT = 99
TOP = 265
...
def label_font():
    for path in (Path(r"C:\Windows\Fonts\calibri.ttf"), Path(r"C:\Windows\Fonts\arial.ttf")):
        if path.exists():
            return ImageFont.truetype(str(path), 30)
...
def make_sheet(index, images):
    ...
    for position, image_path in enumerate(images):
        ...
        y = TOP + row * (CARD_HEIGHT + LABEL_GAP + 36 + ROW_GAP)
...
def main():
    ...
    for index, sheet in enumerate(sheets):
        ...
        picture = paragraph.add_run().add_picture(str(sheet), width=Inches(10.89))
        picture._inline.docPr.set("descr", f"A4 Amazon first-page contact sheet {index + 1} of 4")
    doc.core_properties.title = "Amazon Front Pages - A4 Landscape"
