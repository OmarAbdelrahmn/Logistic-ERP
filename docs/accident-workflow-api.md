...
تبدأ الحالة برقم المرور، ثم رفع تقرير نجم وتسجيل نسب الخطأ لكل طرف. النظام يتابع المستندات والمبالغ ومراحل المطالبة والتواريخ والإشعارات واسترداد الأقساط. رفع المطالبة للتأمين أو شركة المركبة يتم خارج النظام، ثم يسجل المسؤول الرد والمستندات هنا. نسب نجم يدخلها المسؤول بعد مراجعة الملف؛ لا يوجد استخراج آلي/OCR أو اتصال مباشر بنجم أو التأمين.
...
| الحالة | المسار |
| --- | --- |
| خطأ المندوب 100% + حادث بسيط | تقييم الضرر + صورتان على الأقل + تكلفة الإصلاح + ملف سند لأمر بنفس المبلغ المسجل، ثم إثبات انتهاء الإصلاح |
| خطأ المندوب 100% + حادث غير بسيط | تسجيل دفع 2500 ريال وإرفاق ملف سند فتح المطالبة، ثم فتح المطالبة |
...
`accidentNumber` هو أول حقل، يدخله المستخدم، وهو مطلوب وفريد وبحد أقصى 64 حرفًا. وهو مستقل عن `policeReportNumber` الخاص بتقرير المرور/نجم. بقية الحقول المطلوبة هي المركبة، والمندوب، ووقت الحادث، ورقم تقرير المرور/نجم، والخطورة، وحالة قابلية القيادة، وحالة وجود إصابات. عند اختيار وجود إصابات يصبح `injuryDetails` مطلوبًا. الحقول `locationDescription`, `latitude`, `longitude`, `insuranceClaimNumber`, `thirdPartyDetails`, `damageDescription`, `faultAssessment`, و`narrative` اختيارية ويمكن إضافتها لاحقًا. `policeReportNumber` بحد أقصى 150 حرفًا. تاريخ الحادث لا يكون في المستقبل، والمندوب يجب أن تكون له عهدة المركبة وقت وقوع الحادث. تسجيل حادث غير قابل للقيادة يحتفظ بسلوك إنهاء العهدة الموجود بالنظام.
...
| --- | --- | --- |
| 1 AssessFault | مراجعة نجم | `attachmentId` لملف تقرير نجم + `riderFaultPercentage` + `otherParties` |
...
| 24 RejectInstallmentRefund | رفض استرداد الأقساط | `attachmentId` ملف قرار من نوع InsuranceDecision + السبب في `notes` |
...
```json
{
  ...
  "amount": 2500,
  "attachmentId": "<opening-fee-file-guid>"
}
```
...
الملف الواحد حتى 10 MiB. كل أنواع الأدلة والمستندات تقبل PDF أو صورة، بينما النوعان `Image` و`DamagePhoto` يقبلان الصور فقط. الصور المقبولة هي JPG/JPEG وPNG وWebP وGIF وBMP. يتم فحص امتداد الملف ونوعه وبدايته الفعلية، وحساب SHA-256. لا يوجد حد ثابت لعدد مرفقات الحادث أو السطحات.
...
مثال بيانات سطحة: `evidenceType=19`, `description=نقل المركبة لإعادة الفحص`, `fromLocation=المستودع`, `toLocation=مركز الفحص`, `transportedAtUtc=2026-09-03T08:00:00Z`, `amount=150`, مع `file=receipt.pdf` أو `file=receipt.jpg`.
...
```json
{
  ...
  "receiptAttachmentId": "<installment-file-guid>",
